#!/usr/bin/env python
"""Noise-head diagnostic figures for the error-structure report.

Follows the repo plot-generation conventions (see CLAUDE.md): one standalone
vector (PDF) file per plot, no titles, unit-bearing axis labels, print-legible
fonts, tight bbox, a consistent colorblind-safe palette, and a draft-caption
...
plt.rcParams.update({
    ...
    "axes.grid":True,"grid.color":"#e8e8e8","grid.linewidth":.7,"axes.axisbelow":True,
...
# --- Plot: predicted noise scale vs flux ---
...
a.plot(cen,empb,"o-",color=C_EMP,lw=LW_MAIN,ms=5,zorder=5,label="Empirical residual scale")
for m,c,l in [("nll_student_t",C_ST,"Student-t  σ·√dt"),("nll_beta",C_BE,"β-NLL  σ·√dt")]:
    ...
    a.plot(cen,pm,"s--",color=c,lw=LW_REF,ms=4,label=l)
...
fig.tight_layout()
save(fig,"fig_noise_a_scale_vs_flux",
     "Predicted noise scale of the trained NLL heads versus flux, against the empirical residual "
     "scale. Both the Student-t and β-NLL heads are heteroscedastic but rise too shallowly with "
     "flux, under-predicting the residual scale in the high-flux regime.")

# --- Plot: whitened residual tail ---
fig,b=plt.subplots(figsize=(4.8,3.7))
...
for m,c,lab in [("nll_student_t",C_ST,f"Student-t whitened (ν={D['nll_student_t']['nu']:.1f})"),
    ...
    b.plot(ks,[np.mean(np.abs(zc)>k) for k in ks],color=c,lw=LW_MAIN,label=lab)
...
b.set_ylabel("P(|z| > k)  after whitening by model σ  (log scale)")
b.legend(frameon=False)
fig.tight_layout()
save(fig,"fig_noise_b_whitened_tail",
     "Tail exceedance of the residual after whitening by each model's predicted σ. Both heads' "
     "whitened residuals remain heavier-tailed than their assumed noise law past 3 standard "
     "deviations, so the trained heads still under-disperse the extreme increments.")
