...
Read-only w.r.t. the codebase; writes PNGs to this scratchpad dir."""
...
plt.rcParams.update({"font.family":"DejaVu Sans","font.size":10,"axes.grid":True,
    "grid.color":"#e6e6e6","grid.linewidth":.7,"axes.axisbelow":True,
    "axes.edgecolor":"#888","axes.linewidth":.8})

fig,ax=plt.subplots(2,2,figsize=(10.2,7.6))

# Panel A: density with Gaussian & Laplace overlays, log-y (tails)
a=ax[0,0]
lim=8
bins=np.linspace(-lim,lim,120)
a.hist(zc,bins=bins,density=True,color="#cfcfcf",edgecolor="none",label="Empirical residual")
xx=np.linspace(-lim,lim,600)
a.plot(xx,stats.norm.pdf(xx,0,1),color=C_GAU,lw=2,label="Gaussian fit")
a.plot(xx,stats.laplace.pdf(xx,0,1/np.sqrt(2)),color=C_LAP,lw=2,label="Laplace fit")
a.set_yscale("log"); a.set_ylim(1e-4,1)
a.set_xlabel("Standardised residual  (z, unit variance)")
a.set_ylabel("Probability density (log)")
a.set_title("A  Heavy-tailed, non-Gaussian shape",loc="left",fontweight="bold")
a.legend(frameon=False,fontsize=8.5)

# Panel B: tail exceedance P(|z|>k)
b=ax[0,1]
ks=np.linspace(1,6,60)
emp=[np.mean(np.abs(zc)>k) for k in ks]
b.plot(ks,emp,color=C_EMP,lw=2.2,label="Empirical")
b.plot(ks,2*stats.norm.sf(ks),color=C_GAU,lw=2,ls="--",label="Gaussian")
b.plot(ks,np.exp(-np.sqrt(2)*ks),color=C_LAP,lw=2,ls="--",label="Laplace")
for k in (3,):
    b.axvline(k,color="#bbb",lw=.8,ls=":")
b.set_yscale("log"); b.set_ylim(1e-5,1)
b.set_xlabel("Threshold k  (standard deviations)")
b.set_ylabel("P(|z| > k)  (log)")
b.set_title("B  Tail probabilities track Laplace",loc="left",fontweight="bold")
b.legend(frameon=False,fontsize=8.5)

# Panel C: heteroscedasticity — scale vs flux
c=ax[1,0]
rows=[]
...
bt=np.array(rows)
c.scatter(bt[:,0],bt[:,1],color=C_GAU,s=26,zorder=3,label="SD (Gaussian scale)")
c.scatter(bt[:,0],bt[:,2],color=C_LAP,s=26,zorder=3,label="b (Laplace scale)")
...
c.text(.05,.9,f"SD ≈ {q_sd:.2f} + {m_sd:.2f}·flux\n$R^2$=0.99",transform=c.transAxes,
       fontsize=9,va="top")
c.set_xlabel("Predicted nighttime flux  Reco  (µmol m⁻² s⁻¹)")
c.set_ylabel("Local residual scale")
c.set_title("C  Heteroscedasticity: scale ∝ flux",loc="left",fontweight="bold")
c.legend(frameon=False,fontsize=8.5,loc="lower right")

# Panel D: relative uncertainty vs aggregation
dd=ax[1,1]
...
dd.plot(NN,rel*100,"o-",color=C_ACC,lw=2,ms=6,zorder=3,label="Observed relative uncertainty")
...
for N,lbl in pts:
    if N in NN:
        i=list(NN).index(N); dd.annotate(lbl,(N,rel[i]*100),textcoords="offset points",
            xytext=(0,7),fontsize=7.5,ha="center",color="#555")
...
dd.set_title("D  Random error cancels under aggregation",loc="left",fontweight="bold")
dd.legend(frameon=False,fontsize=8.5)

fig.tight_layout(pad=1.4)
fig.savefig(os.path.join(OUT,"fig_error_structure.png"),dpi=170,bbox_inches="tight")
print("wrote fig_error_structure.png")
